...
def login_data():
    ...
    session["id"] = hashlib.sha256(user[0].encode(
        "utf-8") + mail.salt.encode("utf-8")).hexdigest()
...
def get_feed():
    if request.get_json()['action'] == 'getposts':
        ...
        if database.is_liked(session["username"], request.get_json()['postnum']) == False:
            database.add_like(session["username"],
                              request.get_json()['postnum'])
            likes_count += 1
        else:
            database.delete_like(
                session["username"], request.get_json()['postnum'])
            likes_count -= 1
        return dumps({'success': True, 'likes': likes_count})
    elif request.get_json()['action'] == 'newcomment':
        database.add_comment(session['username'], request.get_json()[
                             'postnum'], request.get_json()['content'])
...
def register_data():
    ...
    if database.is_username_exist(request.get_json()['username']) or database.is_email_exist(request.get_json()['email']):
        return dumps({'success': False})
    mail.send_email(request.get_json()['username'], request.get_json()[
